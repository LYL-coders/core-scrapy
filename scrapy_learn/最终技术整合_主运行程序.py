...
class Save_To_Database():
    def __init__(self,
        ...
        self.salarys_text_list = salarys_text_list
        self.job_detail_infos_list = job_detail_infos_list
        self.base_message_list = base_message_list
        # ===========================
        # self.push_self()
    ...
    def save_data_to_sqlite(self):
        ...
        # =============================================================数据库保存
        try:
            print('self.user_image_list', len(self.user_image_list))
            for id in range(len(self.title_list)):
                self.insertDB(id,
                              self.title_list[id],
                              self.user_image_list[id],
                              self.job_detail_infos_list[id],
                              self.salarys_text_list[id],
                              self.base_message_list[id])
    ...
    def insertDB(self, id, title_name, img, job_d_i, salary ,base_message):

        try:
            sql = "insert into niuke_info_list " \
                  "(id,title_name,img,job_detail_infos,salary,base_message) " \
                  "values (?,?,?,?,?,?)"
            self.cursor.execute(sql, (id, title_name, img, job_d_i, salary, base_message))
        except Exception as err:
            print(err)
    ...
    def showDB(self):
        ...
        try:
            ...
            cursor = con.cursor()
            print("%-8s %-32s %-8s %-32s %s" % ("id", "title_name", "img", "job_detail_infos", "salary"))
            cursor.execute("select  id,title_name,img,job_detail_infos,salary,base_message from  phones  orderby id")
...
class MySpider:
    ...
    def startUp(self):
        ...
        # ('//html/body/section/main/div/div[3]/div[3]/div[1]/div/div[2]/div/div[1]/a/div[1]')

        # ------------------------------------------------------------------------------>>>>>>>>>>>>>>>>>>
        # 个人头像集
        element0 = self.browser.find_elements_by_class_name("my-avatar")
        # ------------------------------------------------------------------------------>>>>>>>>>>>>>>>>>>
        # 公司头像集
    ...
    def executeSpider(self):
        ...
        self.startUp()
        print('self.save_path           ', len(self.save_path))
        print('self.image_src_list      ', len(self.image_src_list))
        print('self.title_text_list     ', len(self.title_text_list))
        print('self.salarys_text_list   ', len(self.salarys_text_list))

        database = Save_To_Database(self.save_path,
                                    self.image_src_list,
                                    self.title_text_list,
                                    self.job_detail_infos_list,
                                    self.salarys_text_list,
...
if __name__ == '__main__':
    ...
    # ==================================初始值设置==============================================》》》》》》》》》》》》》》
    save_path = 'download'
    url = "https://www.nowcoder.com/jobs/fulltime/center?recruitType=3&search="  # 牛客
    key = 'python'  # 搜索python对应岗位  可以换成如数据挖掘等。

    """
    别的网站，预选url
    # url = 'https://www.yohobuy.com/product/51085171.html'
    # url = 'https://search.jd.com/Search?keyword=%E6%89%8B%E6%9C%BA&enc=utf-8
    # &suggest=1.his.0.0&wq=&pvid=e664938aaddc4f59ad60f02c2adc77af' #京东
    """
    ...
    #     self.driver.execute_script(js)  # 把搜索边框变为红色
    #     sleep(2)
    #
    #
